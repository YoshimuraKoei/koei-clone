...
flowchart LR
  ...
  subgraph aws["AWS"]
    ...
    OPR[Lambda: vertexAiOpsReporter]
    LOPR[Lambda: lambdaOpsReporter]
  end
...
flowchart TB
  subgraph deploy["serverless.yml to CloudFormation"]
    ...
    L2[scheduler]
    L3[processor]
    L4[vertexAiOpsReporter]
    L5[lambdaOpsReporter]
    R1["EventBridge cron JST 8 and 22"]
    R2["EventBridge cron daily processor"]
    R3["EventBridge cron weekly Vertex AI report"]
    R4["EventBridge cron weekly Lambda report"]
  end
...
## シーケンス: 利用状況の定期通知（vertexAiOpsReporter）
...
sequenceDiagram
  autonumber
  participant EB as EventBridge
  participant Lambda as Lambda vertexAiOpsReporter
  participant CM as Cloud Monitoring
  participant Slack

  EB->>Lambda: スケジュール発火（週 1 回）
  Lambda->>CM: timeSeries.list（requests / tokens / quota exceeded）
  CM-->>Lambda: モデル別メトリクス
  Lambda->>Slack: chat.postMessage（運用保守チャンネル）
```

## シーケンス: Lambda 利用状況の定期通知（lambdaOpsReporter）

EventBridge のスケジュールで Lambda が起動し、CloudWatch Metrics と CloudWatch Logs Insights から各関数の呼び出し回数・実行時間・メモリ使用量を集計して Slack の運用保守チャンネルへ投稿する。

```mermaid
sequenceDiagram
  autonumber
  participant EB as EventBridge
  participant Lambda as Lambda lambdaOpsReporter
  participant CW as CloudWatch Metrics
  participant CWL as CloudWatch Logs Insights
  participant Slack

  EB->>Lambda: スケジュール発火（週 1 回）
  Lambda->>CW: GetMetricData（invocations / errors / throttles / duration / concurrency）
  CW-->>Lambda: 関数別メトリクス
  Lambda->>CWL: StartQuery / GetQueryResults（REPORT 行）
  CWL-->>Lambda: billed duration / memory usage
  Lambda->>Slack: chat.postMessage（運用保守チャンネル）
```
...
```mermaid
flowchart LR
  subgraph fn["Lambda（receiver / scheduler / processor / vertexAiOpsReporter / lambdaOpsReporter）"]
    A[ハンドラ]
    B[try / catch]
    N["notifyOpsError"]
  end
...
**10 本のアラーム**（receiver / scheduler / processor / vertexAiOpsReporter / lambdaOpsReporter の 5 関数 × 2 種別）のイメージ:
...
flowchart TB
  subgraph T["タイムアウト系メトリクス"]
    ...
    t2[SchedulerTimeout]
    t3[ProcessorTimeout]
    t4[VertexAiOpsReporterTimeout]
    t5[LambdaOpsReporterTimeout]
  end
  subgraph O["OOM 系メトリクス"]
    o1[ReceiverOOM]
    o2[SchedulerOOM]
    o3[ProcessorOOM]
    o4[VertexAiOpsReporterOOM]
    o5[LambdaOpsReporterOOM]
  end
